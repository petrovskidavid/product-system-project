...
server.post("/api/login", (req, res) => {
    ...
    internalDb.query("SELECT Email FROM Customers WHERE Email=?", email, (err1, currentCustomer) => {
        ...
        if (currentCustomer.length > 0)
        {
            console.log('\x1b[32m%s\x1b[0m', "Customer exists")
            ...
            internalDb.query("SELECT PASSWORD(?) AS password", preHashPassword, (err2, postHash) => {
                ...
                // Retrieves the hashed password from the internal database
                internalDb.query("SELECT Password as password FROM Customers WHERE Email=?", email, (err3, hashed) => {
                    if (err3)
                        throw (err3)
                    ...
                    if (hashed[0].password == postHash[0].password)
                    {
                        ...
                        console.log('\x1b[32m%s\x1b[0m', "Customer's email and password match")

                        internalDb.query("SELECT Name FROM Customers WHERE Email=?", email, (err4, customer) => {
